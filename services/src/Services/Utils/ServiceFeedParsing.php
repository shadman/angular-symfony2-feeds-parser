	...
	*/
	public function parseJsonData($data){
		...
		foreach($data->products as $items){
			...
			$hc[$h_co]['_key'] = ++$id;
			$hc[$h_co]['id'] = (isset($items->ID))?$items->ID:'';
			$hc[$h_co]['name'] = (isset($items->name))?$items->name:'';
			$hc[$h_co]['description'] = (isset($items->description))?$items->description:'';
			$hc[$h_co]['price']['currency'] = (isset($items->price->currency))?$items->price->currency:'';
			$hc[$h_co]['price']['amount'] = (isset($items->price->amount))?$items->price->amount:'';
			$hc[$h_co]['url'] = (isset($items->URL))?$items->URL:'';
			$hc[$h_co]['image_url'] = (isset($items->images))?$items->images[0]:'';

			$hc[$h_co]['categories'] = array();
			if (isset($items->categories) ){
				foreach ($items->categories as $category) {
					$hc[$h_co]['categories'][] = $category;
				}
			}
